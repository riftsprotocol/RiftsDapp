...
import {
    ArrowUp, ArrowDown, RefreshCw, Loader
} from 'lucide-react';
...
// Create a standard Solana Connection for transaction queries
const standardConnection = new Connection(process.env.NEXT_PUBLIC_SOLANA_RPC_URL!, 'confirmed');
...
export const TradingInterface: React.FC<TradingInterfaceProps> = ({ wallet, rifts, addToast }) => {
    // State
    ...
    useEffect(() => {
        const loadRealPairs = async () => {
            ...
            // RIFTS/SOL pair - uses protocol's native token
            // ✅ WORKING RIFTS TOKEN: 9aJ1VBBGgTptJWYbKwbK2kbAebfgWpv93LCpV5bGwK5P
            // ✅ Meteora Pool: Gk7o7Mmxs3hce9uHRtCSQ9Ku7rDv3eYEc5QyYZPNN1o1 (CP-AMM with liquidity)
            // ✅ Pool Creation TX: 4dU3exDUkHY2TGdmaPLNdhbmW2ojgjQtBvZFMRSopQU84Roywf4Sfj73NQeo4noAr52wFxkVTEvvfjgErj3ccPcV
            // ✅ Test Swap TX: ouEbXNuvuECbfSjxmXS2GPx3iNjUPoBiz2zcoVXRtvsMstw6BmKyTGxByZ3xHRz84P9TQfe9ZjchrWrjNEgLN6j

            // Fetch ALL REAL data from Meteora pool
            let riftsPrice = 0.00005; // Default price
            let riftsTVL = 0.5; // Default TVL
            let riftsVolume24h = 0;
            let riftsChange24h = 0;

            try {
                const poolData = await (dexIntegration as unknown as { getRIFTSMeteoraPoolData: () => Promise<{
                    price: number;
                    tvl: number;
                    riftsReserve: number;
                    solReserve: number;
                    volume24h: number;
                    change24h: number;
                }> }).getRIFTSMeteoraPoolData();

                riftsPrice = poolData.price;
                riftsTVL = poolData.tvl;
                riftsVolume24h = poolData.volume24h;
                riftsChange24h = poolData.change24h;

                debugLog(`✅ Loaded REAL Meteora data: ${riftsPrice.toFixed(8)} SOL/RIFTS, ${riftsTVL.toFixed(4)} SOL TVL, ${riftsVolume24h.toFixed(4)} SOL volume`);
            } catch (error) {
                debugError('Failed to fetch RIFTS pool data:', error);
            }

            // Only add RIFTS/SOL pair if we're showing all trading pairs (not filtering to a specific rift)
            // If rifts array has items, check if it's a filtered single rift or all rifts
            const isSingleRiftFilter = rifts && rifts.length === 1;

            if (!isSingleRiftFilter) {
                pairs.push({
                    base: 'RIFTS',
                    quote: 'SOL',
                    symbol: 'RIFTS/SOL',
                    price: riftsPrice,
                    change24h: riftsChange24h, // REAL 24h change
                    volume24h: riftsVolume24h * 180, // Convert SOL to USD (approximate)
                    high24h: riftsPrice * (1 + Math.abs(riftsChange24h) / 100),
                    low24h: riftsPrice * (1 - Math.abs(riftsChange24h) / 100),
                    mintAddress: '9aJ1VBBGgTptJWYbKwbK2kbAebfgWpv93LCpV5bGwK5P', // ✅ WORKING RIFTS token
                    underlyingMint: 'So11111111111111111111111111111111111111112', // SOL
                    hasMeteoraPool: true, // ✅ Pool has liquidity and swaps work!
                    tvl: riftsTVL // REAL SOL liquidity from Meteora pool
                });
            }
            ...
            for (const rift of rifts) {
                ...
                if (shouldInclude) {
                    try {
                        ...
                        const backingRatio = (riftData.backingRatio || 0) / 10000 || 1;
                        const currentPrice = backingRatio;
                        ...
                        riftPairs.push({
                            base: `r${riftData.underlying || 'UNKNOWN'}`,
                            quote: riftData.underlying || 'UNKNOWN',
                            symbol: `r${riftData.underlying || 'UNKNOWN'}/${riftData.underlying || 'UNKNOWN'}`,
                            price: currentPrice,
                            change24h: change24h,
                            volume24h: volume24h,
                            ...
                            underlyingMint: riftData.underlyingMint || 'So11111111111111111111111111111111111111112', // Default to SOL
                            hasMeteoraPool: riftData.hasMeteoraPool || false,
                            tvl: riftData.tvl || 0
                        });

                        debugLog(`✅ Added trading pair: ${riftData.symbol} (TVL: ${riftData.tvl}, Meteora: ${riftData.hasMeteoraPool})`);
                    } catch (error) {
            ...
            // Set final pairs (RIFTS/SOL + rift pairs)
            const allPairs = [...pairs, ...riftPairs];
            setRealPairs(allPairs);

            // Select first available pair if none selected
    ...
    useEffect(() => {
        ...
        const loadRealTransactions = async () => {
            ...
            try {
                ...
                // Load REAL trades from Meteora pool for RIFTS/SOL
                if (selectedPair.base === 'RIFTS') {
                    try {
                        debugLog('📜 Fetching real trades from Meteora pool...');
                        ...
                        debugLog(`✅ Loaded ${transactions.length} real trades from Meteora`);
                    } catch (error) {
                        debugError('Error loading Meteora trades:', error);
                    }
                } else if (selectedPair.riftAddress) {
                    try {
                        // OPTIMIZATION: Only fetch from volume tracking (instant), skip slow blockchain queries on initial load
                        const riftData = rifts.find(r => (r as unknown as { id?: string }).id === selectedPair.riftAddress) as unknown as { id?: string; volume24h?: number };
                        if (riftData) {
                            const volumeEntries = (riftsService as unknown as { getVolumeHistory: (address: string) => unknown[] }).getVolumeHistory(selectedPair.riftAddress);

                            volumeEntries.forEach((entry, index) => {
                                const entryData = entry as unknown as { amount?: number; timestamp?: number; participant?: string };
                                if ((entryData.amount || 0) > 0) {
                                    transactions.push({
                                        signature: `volume_${entryData.timestamp || Date.now()}_${index}`,
                                        type: 'wrap',
                                        token: selectedPair.base,
                                        amount: entryData.amount || 0,
                                        price: selectedPair.price,
                                        timestamp: entryData.timestamp || Date.now(),
                                        user: entryData.participant || 'anonymous',
                                        fee: (entryData.amount || 0) * 0.007
                                    });
                                }
                            });
                        }
                    } catch (error) {
                        debugError('Error loading transaction history:', error);
                    }
                }
    ...
    // Generate REAL price chart data from blockchain trades
    useEffect(() => {
        const generatePriceData = async () => {
            ...
            try {
                ...
                // Build price points from REAL trades ONLY - no fake data
                if (selectedPair.base === 'RIFTS' && realTransactions.length > 0) {
                    ...
                    // No trades - show single point at current price
                    pricePoints.push({
                        timestamp: Date.now(),
                        price: selectedPair.price
                    });
        ...
        generatePriceData();
        const interval = setInterval(generatePriceData, 60000);
    ...
    const handleRealTrade = async () => {
        ...
        try {
            ...
            if (inputCurrency === 'base') {
                // User entered base currency (RIFTS)
                tradeAmount = inputAmount;
                // When buying: inputAmount is in base, need to convert to quote for actual swap
                // When selling: inputAmount is in base, use directly
                actualInputAmount = tradeType === 'buy'
                    ? inputAmount * selectedPair.price  // Convert RIFTS to SOL
                    : inputAmount; // Use RIFTS directly
            } else {
                // User entered quote currency (SOL)
                tradeAmount = inputAmount / selectedPair.price; // Convert to base for display
                // When buying: inputAmount is in quote (SOL), use directly
                // When selling: this shouldn't happen (you can't sell SOL for RIFTS by entering SOL amount)
                actualInputAmount = inputAmount; // Use SOL directly
            }
            ...
            if (selectedPair.base === 'RIFTS') {
                ...
                if (tradeType === 'buy') {
                    ...
                    outputMint = '9aJ1VBBGgTptJWYbKwbK2kbAebfgWpv93LCpV5bGwK5P'; // ✅ WORKING RIFTS token
                } else {
                    inputMint = '9aJ1VBBGgTptJWYbKwbK2kbAebfgWpv93LCpV5bGwK5P'; // ✅ WORKING RIFTS token
                ...
                console.log('✅ Using rift mints - Input:', riftData.underlyingMint, 'Output:', riftData.riftMint);
                
                if (tradeType === 'buy') {
                    // Buy wrapped token with underlying token
                    inputMint = riftData.underlyingMint; // e.g., SOL
                    outputMint = riftData.riftMint; // e.g., rSOL
                } else {
                    // Sell wrapped token for underlying token
                    inputMint = riftData.riftMint; // e.g., rSOL
                    outputMint = riftData.underlyingMint; // e.g., SOL
                }
            } else {
            ...
            const inputAmountLamports = Math.floor(actualInputAmount * 1e9);

            let signature: string;

            // Check if this is RIFTS/SOL trading (use Meteora directly)
            if (selectedPair.base === 'RIFTS') {
                debugLog(`🌊 Using Meteora CP-AMM for RIFTS/SOL: ${inputAmountLamports} ${inputMint} -> ${outputMint}`);

                // Update user with Meteora routing information
                addToast?.(
                    `🌊 Executing trade via Meteora CP-AMM...\n` +
                    `Pool: Gk7o7Mmxs3hce9uHRtCSQ9Ku7rDv3eYEc5QyYZPNN1o1`,
                    'pending'
                );

                // Use Meteora direct swap for RIFTS token
                signature = await riftsService.executeMeteoraSwap({
                    poolAddress: 'Gk7o7Mmxs3hce9uHRtCSQ9Ku7rDv3eYEc5QyYZPNN1o1', // RIFTS/SOL pool
                    inputMint,
                    outputMint,
                ...
                if (riftData.liquidityPool && riftData.liquidityPool !== '11111111111111111111111111111111') {
                    ...
                    debugLog(`   Pool address: ${riftData.liquidityPool}`);
                    ...
                    signature = await riftsService.executeMeteoraSwap({
                        poolAddress: riftData.liquidityPool,
                        inputMint,
                        outputMint,
                        amount: inputAmountLamports,
                        slippageBps: 300, // 3% slippage
            ...
            console.log('📊 Swap Details:', {
                ...
                inputAmountLamports,
                expectedOutput: tradeType === 'buy' ? (actualInputAmount / selectedPair.price) : (actualInputAmount * selectedPair.price),
                signature
            });
            ...
            if (tradeType === 'buy') {
                // Buying: SOL -> rSOL (or similar)
                outputAmount = actualInputAmount / currentPrice;
                outputToken = selectedPair.base;
                inputToken = selectedPair.quote;
            } else {
                // Selling: rSOL -> SOL (or similar)
                outputAmount = actualInputAmount * currentPrice;
                ...
                try {
                    ...
                    if (!poolAddress) {
                        ...
                        const localInputMint = (selectedPair.base === 'RIFTS') ?
                            (tradeType === 'buy' ? 'So11111111111111111111111111111111111111112' : '9aJ1VBBGgTptJWYbKwbK2kbAebfgWpv93LCpV5bGwK5P') :
                            (tradeType === 'buy' ?
                        ...
                        const localOutputMint = (selectedPair.base === 'RIFTS') ?
                            (tradeType === 'buy' ? '9aJ1VBBGgTptJWYbKwbK2kbAebfgWpv93LCpV5bGwK5P' : 'So11111111111111111111111111111111111111112') :
                            (tradeType === 'buy' ?
                                (rifts.find(r => (r as any).id === selectedPair.riftAddress) as any)?.riftMint :
                                (rifts.find(r => (r as any).id === selectedPair.riftAddress) as any)?.underlyingMint
                            );
    ...
    // Calculate REAL total with fees
    const calculateRealTotal = () => {
        const amountNum = parseFloat(amount) || 0;
        if (amountNum === 0) {
            return { baseAmount: 0, quoteAmount: 0, subtotal: 0, fee: 0, total: 0 };
        }

        // Use the REAL price from selectedPair (fetched from blockchain/pool)
        // selectedPair.price is in USD, but we need SOL/token exchange rate
        // For rift tokens: 1 rift token ~= price in USD / SOL price in USD
        // Assume SOL = $180 for devnet
        const SOL_PRICE_USD = 180;
        const tokenPriceInSOL = selectedPair ? (selectedPair.price / SOL_PRICE_USD) : 0.001;

        let baseAmount: number;
        let quoteAmount: number;

        if (inputCurrency === 'base') {
            // User entered base currency (RIFTS/rSOL) - SELLING
            baseAmount = amountNum;
            // Selling X tokens, get Y SOL
            quoteAmount = amountNum * tokenPriceInSOL;
        } else {
            // User entered quote currency (SOL) - BUYING
            quoteAmount = amountNum;
            // Buying with X SOL, get Y tokens
            baseAmount = amountNum / tokenPriceInSOL;
        }

        // Fee is 0.7% on the output amount
        const fee = quoteAmount * 0.007;

        if (inputCurrency === 'base') {
            // Selling: subtract fee from what you receive
            return {
                baseAmount,
            ...
            // Buying: add fee to what you pay
            return {
                baseAmount,
                quoteAmount,
                subtotal: quoteAmount,
    ...
    const RealPriceChart = () => {
        ...
        useEffect(() => {
            ...
            ctx.textAlign = 'left';
            ctx.fillText(`$${selectedPair?.price.toFixed(8) || '0'}`, padding, padding - 10);
    ...
    return (
        <div className="overflow-hidden border border-gray-800 bg-gray-900/50 backdrop-blur-sm rounded-xl">
            {/* Header */}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                        ...
                        <div className="relative">
                            <select
                                ...
                                className="w-full sm:w-auto px-4 py-2 text-white bg-gray-900 border border-gray-700 rounded-lg"
                            >
                                {realPairs.map((pair, index) => (
                                    <option key={pair.riftAddress || pair.symbol || `pair-${index}`} value={pair.symbol}>
                                        {pair.symbol} {pair.hasMeteoraPool ? '🌊' : ''} {pair.tvl && pair.tvl > 0 ? `(${pair.tvl.toFixed(2)} SOL)` : ''}
                                    </option>
                                ))}
                            </select>
                        </div>
                        ...
                        <div className="flex items-center gap-3 md:gap-6 flex-wrap">
                            ...
                            <div>
                                ...
                                <div className={`text-lg font-bold ${displayPair.change24h >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                                    {displayPair.change24h >= 0 ? <ArrowUp className="inline w-4 h-4" /> : <ArrowDown className="inline w-4 h-4" />}
                                    {Math.abs(displayPair.change24h).toFixed(2)}%
                                </div>
                            </div>
                            <div>
                                <div className="text-xs text-gray-400">24h Volume</div>
                                <div className="text-sm text-white">${displayPair.volume24h.toLocaleString()}</div>
                            </div>
                            {displayPair.hasMeteoraPool && (
                                <div>
                                    ...
                                    <div className="flex items-center gap-1 text-sm">
                                        ...
                                        {displayPair.tvl && displayPair.tvl > 0 && (
                                            <span className="text-green-400">({displayPair.tvl.toFixed(2)} SOL)</span>
                                        )}
                                    </div>
                                </div>
                            )}
                            <div>
                                ...
                                <div className="text-sm text-white">
                                    {getUserBalance(displayPair.base).toFixed(4)} {displayPair.base}
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 p-4">
                ...
                <div className="lg:col-span-8">
                    <div className="p-4 rounded-lg bg-gray-800/30 h-64 md:h-80">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="font-medium text-white">{displayPair.symbol} Price Chart</h3>
                    ...
                    {/* Trading Form */}
                    <div className="p-4 mt-4 rounded-lg bg-gray-800/30">
                                ...
                                <div className="flex gap-2 mt-2">
                                    {[25, 50, 75, 100].map((percent) => (
                                        <button
                                            ...
                                            className="flex-1 py-1 text-xs text-gray-400 transition-colors bg-gray-800 rounded hover:bg-gray-700 hover:text-white"
                                        >
                                            {percent}%
                                        </button>
                                    ))}
                            ...
                            <div className="p-3 rounded-lg bg-gray-900/50">
                                <div className="space-y-2 text-sm">
                                    <div className="flex justify-between">
                                        ...
                                        <span className="text-white">${displayPair.price.toFixed(6)}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-400">{inputCurrency === 'base' ? 'You Input' : 'You Input'}</span>
                                        <span className="text-white">{amount || '0'} {inputCurrency === 'base' ? displayPair.base : displayPair.quote}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-400">{inputCurrency === 'base' ? 'Total Cost' : 'You Get'}</span>
                                        <span className="text-white">
                                            {inputCurrency === 'base'
                                                ? `${calculateRealTotal().quoteAmount.toFixed(6)} ${displayPair.quote}`
                                                : `${calculateRealTotal().baseAmount.toFixed(6)} ${displayPair.base}`
                                            }
                                        </span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-400">Fee (0.7%)</span>
                                        <span className="text-orange-400">{calculateRealTotal().fee.toFixed(6)} {displayPair.quote}</span>
                                    </div>
                                    <div className="flex justify-between pt-2 border-t border-gray-700">
                                        <span className="font-medium text-gray-400">Total</span>
                                        <span className="font-bold text-white">{calculateRealTotal().total.toFixed(6)} {displayPair.quote}</span>
                                    </div>
                                </div>
                            </div>
                ...
                {/* REAL Order Book & Transactions */}
                <div className="lg:col-span-4 space-y-4">
                    {/* REAL Order Book */}
                    <div className="p-4 rounded-lg bg-gray-800/30">
                        <h3 className="mb-3 text-sm font-medium text-white">Live Order Book</h3>
                        
                        <div className="space-y-1">
                            <div className="grid grid-cols-3 pb-2 text-xs text-gray-400">
                                <div>Price ({displayPair.quote})</div>
                                <div className="text-right">Amount ({displayPair.base})</div>
                            ...
                            <div className="space-y-0.5">
                                {orderBook.asks.slice(0, 8).reverse().map((ask, i) => (
                                    <div key={i} className="grid grid-cols-3 text-xs cursor-pointer hover:bg-gray-800/50">
                                        <div className="text-red-400">{ask.price.toFixed(6)}</div>
                                        <div className="text-right text-white">{ask.amount.toFixed(2)}</div>
                            ...
                            {/* Current Price */}
                            <div className="py-2 border-gray-700 border-y">
                            ...
                            <div className="space-y-0.5">
                                {orderBook.bids.slice(0, 8).map((bid, i) => (
                                    <div key={i} className="grid grid-cols-3 text-xs cursor-pointer hover:bg-gray-800/50">
                                        ...
                                        <div className="text-right text-white">{bid.amount.toFixed(2)}</div>
                    ...
                    <div className="p-4 rounded-lg bg-gray-800/30">
                        <div className="flex items-center justify-between mb-3">
                            ...
                            <div className="text-xs text-gray-400">
                                {realTransactions.length} real transactions
                            </div>
                        </div>
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="space-y-0.5 max-h-64 overflow-y-auto">
                                {realTransactions.map((tx) => (
                                    <div 
                                        key={tx.signature} 
                                        className="grid grid-cols-3 text-xs cursor-pointer hover:bg-gray-800/50"
                                        title={`Signature: ${tx.signature}`}
                                    >
                                        <div className={tx.type === 'wrap' || tx.type === 'buy' ? 'text-green-400' : 'text-red-400'}>
                                            ${tx.price.toFixed(6)}
                                        </div>
                                        <div className="text-right text-white">{tx.amount.toFixed(2)}</div>
