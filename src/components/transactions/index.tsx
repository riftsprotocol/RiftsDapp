...
export const CreateRiftModal: React.FC<CreateRiftModalProps> = ({ 
  ...
  const [burnFee, setBurnFee] = useState('45'); // Default 45%
  ...
  const handleCreate = async () => {
    ...
    if (burnFeeNum < 0 || burnFeeNum > 45) {
      return;
    }

    if (partnerFeeNum < 0 || partnerFeeNum > 5) {
      return;
    }

    if (partnerFeeNum > 0 && !partnerWallet) {
      return;
    }
    ...
    try {
      const result = await createRift({
        ...
        partnerWallet: partnerFeeNum > 0 ? partnerWallet : undefined
      });
