...
export class HybridOracleManager {
  ...
  // Execute oracle-driven rebalance
  async executeOracleRebalance(params: {
    ...
    try {
      const { riftId, newPrice, confidence } = params;
    ...
    try {
      ...
      for (const rift of allRifts) {
        ...
        if (signal.shouldRebalance && signal.confidence > 0.8) {
          ...
          const underlyingPrice = await this.getAggregatedPrice(rift.underlyingMint);
          
          await this.executeOracleRebalance({
            riftId: rift.id,
            underlyingMint: rift.underlyingMint,
            newPrice: underlyingPrice.price,
            confidence: underlyingPrice.confidence * 100
          });
        }
      }
    } catch (error) {
